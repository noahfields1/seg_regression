...
msh.SetMeshOptions('MeshWallFirst',[1])
msh.SetMeshOptions('NoMerge',[True])
msh.SetMeshOptions('NoBisect',[True])
msh.SetMeshOptions('Epsilon',[1e-8])
...
if BOUNDARY_LAYER:
    ...
    for v in WALL_IDS:
        msh.SetBoundaryLayer(0,int(v),0,5,[0.75,0.65,0])
    for v in CAP_IDS:
        msh.SetBoundaryLayer(0,int(v),0,5,[0.75,0.65,0])
